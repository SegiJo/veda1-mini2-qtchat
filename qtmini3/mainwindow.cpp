...
void MainWindow::loadAllChatRooms()
{
    ...
    ui->chatTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Fixed);  // 첫 번째 열 고정
    ui->chatTable->setColumnWidth(0, 150);  // 첫 번째 열의 고정 크기 설정
    ui->chatTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);  // 두 번째 열 확장
    ui->chatTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::Stretch);  // 세 번째 열 확장
}
